...
class Scoreboard {
  ...
  addScoreToHighScores(score) {
    if (this.sortedScores.length < 5 || score > this.sortedScores[4].score) {
      ...
      $('.form').append(`<form><label>Sweet Dino Jumping Scores! Get Ready to be Famous!</label><input id="name" type="text"></input><input id="submit" class="btn btn-score" type="submit"></input></form>`);
      $('#submit').on('click', (e) => {
        e.preventDefault();
